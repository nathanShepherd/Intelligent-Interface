...
import datetime# for saving models with date information
import random
import numpy as np
...
class Memory:
    ...
    def push_sample(self,state, action, reward, state_next, done):
        #container is a (mostly) presorted vector of transitions
        ...
        if len(self.container) > self.capacity:
            ...
            del self.container[int(random.random() * len(self.container))]
...
class DQN:
    def __init__(
        ...
        self.epsilon = 1 - random_action_chance
        self.epsilon_decay = random_action_decay
        self.epsilon_max = epsilon_max
        self.use_decay_epsilon=use_decay_epsilon
    ...
    '''less likely to randomly take action relative to get_action'''
    ...
    def get_action(self, state):
        ...
        if random.random() < self.epsilon:#less than epsilon?
    ...
    def train(self):
        ...
        #decrement chance of taking random action
        if self.use_decay_epsilon:
            self.training_iters += 1
            self.epsilon = self.epsilon_max * np.tanh(self.training_iters/self.epsilon_decay)
